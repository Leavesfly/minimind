# 15 - 训练数据格式详解：每种「食材」的包装规范

> 不同训练阶段需要不同格式的数据，就像不同的烹饪方式需要不同形状的食材。本文详细说明每种数据的格式规范和设计原因。
...
## 15.2 预训练数据：`pretrain_t2t_mini.jsonl`——散装原材料
...
## 15.3 SFT 数据：`sft_t2t_mini.jsonl`——精心搭配的多轮对话
